  ...
  return (
    <div className="settings-container">
      ...
      <div className="settings-content">
        ...
        {user.role === 'teacher' && (
          <div className="settings-section">
            <h2>📥 Upload Dataset (Teacher Only)</h2>

            <div className="setting-item">
              <label>Upload CSV, JSON, or TXT files</label>
              <input
                type="file"
                accept=".csv,.json,.txt"
                multiple
            ...
            <div className="info-box">
              <p>Only teachers can upload files. Uploaded files are saved directly into the dataset folder used by the RAG pipeline.</p>
              <p>After uploading dataset files, click Rebuild Knowledge Base to clear the existing ChromaDB collection and reload documents.</p>
            </div>

            {uploadMessage && <div style={{ marginTop: '0.75rem', color: '#2e7d32' }}>{uploadMessage}</div>}
            {rebuildMessage && <div style={{ marginTop: '0.75rem', color: '#2e7d32' }}>{rebuildMessage}</div>}
          </div>
        )}
